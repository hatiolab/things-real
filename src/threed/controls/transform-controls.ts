...
export default class TransformControls extends THREE.Object3D {
  ...
  set mode(mode) {
    ...
    for (var type in this.gizmo) this.gizmo[type].visible = type === this._mode;
  ...
  update() {
    ...
    this.camRotation.setFromRotationMatrix(
      this.tempMatrix.extractRotation(this.camera.matrixWorld)
    );

    var nscale =
      (this.worldPosition.distanceTo(this.camPosition) / 6) * this.size;
  ...
  onPointerHover(event) {
    ...
    var intersect = this.intersectObjects(
      pointer,
      this.gizmo[this.mode].pickers.children
    );
    ...
    if (pointer.button === 0 || pointer.button === undefined) {
      ...
      if (intersect) {
        ...
        if (planeIntersect) {
          ...
          this.parentRotationMatrix.extractRotation(
            this.object.parent.matrixWorld
          );
          this.parentScale.setFromMatrixScale(
            this.tempMatrix.getInverse(this.object.parent.matrixWorld)
          );
  ...
  onPointerMove(event) {
    ...
    var planeIntersect = this.intersectObjects(pointer, [
      this.gizmo[this.mode].activePlane
    ]);
    ...
    if (this.mode === "translate") {
      ...
      if (this.space === "local") {
        this.point.applyMatrix4(
          this.tempMatrix.getInverse(this.worldRotationMatrix)
        );
      ...
      if (this.space === "world" || this.axis.search("XYZ") !== -1) {
        ...
        this.point.applyMatrix4(
          this.tempMatrix.getInverse(this.parentRotationMatrix)
        );
      ...
      if (this.space === "local") {
        if (this.axis === "XYZ") {
          var nscale =
            1 +
            this.point.y /
              Math.max(this.oldScale.x, this.oldScale.y, this.oldScale.z);
          ...
          if (this.axis === "X")
            this.object.scale.x =
              this.oldScale.x * (1 + this.point.x / this.oldScale.x);
          if (this.axis === "Y")
            this.object.scale.y =
              this.oldScale.y * (1 + this.point.y / this.oldScale.y);
          if (this.axis === "Z")
            this.object.scale.z =
              this.oldScale.z * (1 + this.point.z / this.oldScale.z);
        }
      }
      ...
      if (this.axis === "E") {
        ...
        this.tempVector.applyMatrix4(
          this.tempMatrix.getInverse(this.lookAtMatrix)
        );
        ...
        this.quaternionE.setFromAxisAngle(
          this.eye,
          rotation.z - this.offsetRotation.z
        );
        ...
        if (this.rotationSnap !== null) {
          this.quaternionX.setFromAxisAngle(
            this.unitX,
            Math.round(
              (rotation.x - this.offsetRotation.x) / this.rotationSnap
            ) * this.rotationSnap
          );
          this.quaternionY.setFromAxisAngle(
            this.unitY,
            Math.round(
              (rotation.y - this.offsetRotation.y) / this.rotationSnap
            ) * this.rotationSnap
          );
          this.quaternionZ.setFromAxisAngle(
            this.unitZ,
            Math.round(
              (rotation.z - this.offsetRotation.z) / this.rotationSnap
            ) * this.rotationSnap
          );
        } else {
          this.quaternionX.setFromAxisAngle(
            this.unitX,
            rotation.x - this.offsetRotation.x
          );
          this.quaternionY.setFromAxisAngle(
            this.unitY,
            rotation.y - this.offsetRotation.y
          );
          this.quaternionZ.setFromAxisAngle(
            this.unitZ,
            rotation.z - this.offsetRotation.z
        ...
        if (this.axis === "X")
          this.quaternionXYZ.multiplyQuaternions(
            this.quaternionXYZ,
            this.quaternionX
          );
        if (this.axis === "Y")
