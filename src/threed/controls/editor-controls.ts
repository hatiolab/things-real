...
export default class EditorControls extends THREE.EventDispatcher {
  ...
  onMouseMove(event) {
    ...
    if (this.state === STATE.ROTATE) {
      this.rotate(
        new THREE.Vector3(
          -movementX * this.rotationSpeed,
          -movementY * this.rotationSpeed,
          0
        )
      );
  ...
  onMouseOut(event) {
    var { left, top, right, bottom } = this.domElement.getBoundingClientRect();
  ...
  touchMove(event) {
    ...
    switch (event.touches.length) {
      case 1:
        ...
        this.rotate(
          touches[0]
            .sub(getClosest(touches[0], this.prevTouches))
      ...
      case 2:
        ...
        var offset1 = touches[1]
          .clone()
          .sub(getClosest(touches[1], this.prevTouches));
        offset0.x = -offset0.x;
        offset1.x = -offset1.x;
