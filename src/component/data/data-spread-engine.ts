...
function buildSpreadRules(rules: DataSpreadModel[], owner) {
  if (!rules) {
    return [];
  }

  return (rules || []).map(rule => {
    return new DataSpreadRule(rule, owner);
  });
}
...
export default class DataSpreadEngine {
   ...
   */
  reset() {
    this.spreadRules = buildSpreadRules(this.owner.mappings, this.owner);
  }
    ...
    this.spreadRules &&
      this.spreadRules.forEach(rule => {
        try {
          ...
          if (target == "(key)") {
            let targets = Object.keys(accessor || {})
              .map(key => root.findOrCreate(key))
              .filter(t => !!t);
            targets.forEach(
              target =>
                (target[property] = rule.evaluator(accessor[target.get("id")], [
                  target
                ]))
            );
            ...
            if (!target.endsWith("]"))
              throw String(
                "rule target should end with ']' to designate property-id.(" +
                  target +
                  ")"
              );
            ...
            if (accessor instanceof Array) {
              ...
              throw String(
                "rule data should be an object to target property-id.(" +
                  this.owner.data +
                  ")"
              );
            }
