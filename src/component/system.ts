...
import info from '../real/info'
import FPS from '../util/fps'

const source = [
  'things-real', 'user-info',
  'fps', 'time', 'elapse-time', 'memory'
].join(',').split(',')
...
export default class SystemDataSource extends DataSource {
  static _image

  static get image() {
    if (!SystemDataSource._image) {
      SystemDataSource._image = new Image()
      SystemDataSource._image.src = RANDOM_IMAGE
    }

    return SystemDataSource._image
  }
  ...
  static readonly NATURE = {
    ...
    properties: [{
      ...
      name: 'source',
      property: {
        options: source.map(source => ({ display: source, value: source }))
      }
    }, {
      type: 'number',
      label: 'period',
  ...
  detect() {
    ...
    switch (source) {
      ...
      case 'time':
        ...
        break
      case 'elapse-time':
        this.data = performance.now() - this._startTime
        break
      case 'memory':
        let {
          jsHeapSizeLimit = 0,
...
Component.register(SystemDataSource.type, SystemDataSource);
